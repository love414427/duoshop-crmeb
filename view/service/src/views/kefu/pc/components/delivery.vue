<template>
  ...
  >
    <div>
      <el-form ref="formValidate" :model="formValidate" :rules="ruleInline" label-width="100px">
        <el-form-item label="选择类型：" class="form-item" label-position="right">
          <el-radio-group v-model="formValidate.gender">
            <el-radio v-for="(item,index) in radioList" :key="index" :label="item.key">{{ item.title }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="formValidate.gender == 1" :key="'test0'" label="发货类型：" class="form-item" label-position="right">
          <el-radio-group v-model="formValidate.shipStatus">
            <el-radio v-for="(item,index) in shipType" :key="index" :label="item.key">{{ item.title }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <!--  发货手动填写  -->
        <div v-if="formValidate.gender == 1 && formValidate.shipStatus == 1" :key="'test1'">
          <el-form-item label="快递公司：" prop="logisticsCode" class="form-item" label-position="right">
            <el-select v-model="formValidate.logisticsCode" filterable placeholder="请选择" :label-in-value="true" style="width: 100%" @on-change="bindChange">
              <el-option v-for="(item,index) in logisticsList" :key="index" :value="item.code">{{ item.value }}</el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="快递单号：" prop="number" class="form-item" label-position="right">
            <el-input v-model="formValidate.number" placeholder="请输入快递单号" style="width: 100%" />
          </el-form-item>
          <el-form-item label="" class="form-item" label-position="right">
        ...
        <div v-if="formValidate.gender == 1 && formValidate.shipStatus == 2" :key="'test2'">
          <el-form-item label="快递公司：" prop="logisticsCode" class="form-item" label-position="right">
            <el-select v-model="formValidate.logisticsCode" placeholder="请选择" style="width: 100%" filterable :label-in-value="true" @on-change="bindChange">
              <el-option v-for="(item,index) in logisticsList" :key="index" :value="item.code">{{ item.value }}</el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="orderTempList.length>0" label="电子面单：" class="form-item" label-position="right">
            <el-select v-model="formValidate.electronic" placeholder="请选择电子面单" style="width: 80%">
              <el-option v-for="(item,index) in orderTempList" :key="index" :value="item.temp_id">{{ item.title }}</el-option>
            </el-select>
            ...
            <viewer
              ...
              @inited="inited"
            >
              <img v-for="src in orderTempList" :key="src.id" :src="src.pic" class="image">
            </viewer>
          </el-form-item>
          <el-form-item label="寄件人姓名：" prop="sendName" class="form-item" label-position="right">
            <el-input v-model="formValidate.sendName" placeholder="请输入寄件人姓名" style="width: 100%" />
          </el-form-item>
          <el-form-item label="寄件人电话：" prop="sendPhone" class="form-item" label-position="right">
            <el-input v-model="formValidate.sendPhone" placeholder="请输入寄件人电话" style="width: 100%" />
          </el-form-item>
          <el-form-item label="寄件人地址：" prop="sendAddress" class="form-item" label-position="right">
            <el-input v-model="formValidate.sendAddress" placeholder="请输入寄件人地址" style="width: 100%" />
          </el-form-item>
        </div>
        <!--  送货  -->
        <div v-if="formValidate.gender == 2" :key="'test3'">
          <el-form-item label="选择送货人：" class="form-item" label-position="right">
            <el-select v-model="formValidate.postPeople" placeholder="选择送货人" style="width: 100%">
              <el0option v-for="(item,index) in deliveryList" :key="index" :value="item.id">{{ item.nickname }}</el0option>
            </el-select>
          </el-form-item>
        </div>
        <div v-if="formValidate.gender == 3">
          <el-form-item label="备注：" props="msg" class="form-item" label-position="right">
            <el-input v-model="formValidate.msg" placeholder="备注" />
          </el-form-item>
        </div>
